    ...
    def _write_layer(self):
        ...
        for layer in self._srim.target.layers:
            for element in layer.elements:
                ...
                if layer.elements[element]['unique_name'] not in self.element_position :
                   self.element_position.append(layer.elements[element]['unique_name'])
        ...
        for layer_index, layer in enumerate(self._srim.target.layers, start=1):
            ...
            for descriptor in self.element_position :
                e_index=1
                h_index=len(layer.elements)
                for system in layer.elements :
                    if descriptor==layer.elements[system]['unique_name'] :
                        layer_str += ' {} '.format(layer.elements[system]['stoich'])
                        e_index+=999
                    elif h_index==e_index :
                        layer_str += ' 0.0 '
                    else :
                        e_index+=1
                #layer_str += ' 0.0' * (self.srim_num_elements - element_index - len(layer.elements))
            layer_str += self.newline
        layer_str=layer_str[:layer_str.rfind('\r\n')]
